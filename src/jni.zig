...
fn flags_set() void {
    ...
    if (!ok(linux.syscall2(.prctl, c.PR_SET_DUMPABLE, 0))) {
        @panic("Dumpable failed");
    }
    if (!ok(linux.syscall2(.setrlimit, c.RLIMIT_CORE, @intFromPtr(&lim)))) {
        @panic("set rlimit failed");
    }
}
...
export fn JNI_OnLoad(vm: *c.JavaVM, reserved: ?*anyopaque) c.jint {
    ...
    std.debug.panic("seccomp failed: {}", .{err});
    };
